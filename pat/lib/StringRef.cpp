...
bool pat::StrToULongLong(StringRef pStr, unsigned int pRadix,
...
/// @return false only if any error occurs.
bool pat::StrToLongLong(StringRef pStr, unsigned int pRadix,
                             long long& pResult)
{
...
StringRef::StringRef(const char *pStr)
  : m_Data(pStr) {
  assert(pStr && "StringRef cannot be built from a NULL argument");
